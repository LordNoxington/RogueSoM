...
--[[    
    if UnitPower("player") < 80 then
      if targetclass ~= "Druid" and targetclass ~= "Shaman" then
      ...
      elseif (targetclass == "Druid" or targetclass == "Shaman") and GetInventoryItemID("player",17) ~= 28310 then
        EquipItemByName(28768, 17) -- wound weapon
      end
    end
]]
 ...
 local function Opener()
  ...
  local function Dps()
    if UnitAffectingCombat("player") and UnitExists("target") and UnitCanAttack("player","target") and not buff(Stealth,"player") and not buff(Vanish,"player") then
      ...
      if castable(ExposeArmor, "target") and health("target") >= 40 and myComboPoints >= 4 and (cooldown(KidneyShot) >= 16 or isElite("target")) and (targetclass == "Warlock" or targetclass == "Priest" or targetclass == "Mage") then
